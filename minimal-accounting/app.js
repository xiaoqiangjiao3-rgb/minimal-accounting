...
class AccountingApp {
    ...
    bindEvents() {
        ...
        if (goalSubmitBtn) {
            goalSubmitBtn.addEventListener('click', () => this.setGoal());
        }
        var goalInput = document.getElementById('goal-input');
        if (goalInput) {
            goalInput.addEventListener('keypress', e => { if (e.key === 'Enter') this.setGoal(); });
        }

        // 管理分类 - 改用 openCategoryManager 动态绑定
        ...
        if (healthHeader) {
            healthHeader.addEventListener('click', () => {
                var healthCard = document.getElementById('health-card');
                if (healthCard) healthCard.classList.toggle('expanded');
            });
        }
    }
    ...
    renderHeaderBalance() {
        // 已移除：header-balance 元素不存在于当前 UI
    }
    ...
    renderSummaryCards() {
        ...
        document.getElementById('balance-value').style.color = balance >= 0 ? 'var(--income)' : 'var(--expense)';
        // 按比例调整汇总条宽度
    ...
    renderQuickCats(type) {
        ...
        }).join('') + '<button class="cat-chip cat-custom-btn" id="custom-cat-btn">+ 自定义</button>';
        container.querySelectorAll('.cat-chip:not(.cat-custom-btn)').forEach(function(chip) {
            chip.addEventListener('click', function(e) { app.quickAdd(e.currentTarget.dataset.cat); });
        });
        var customBtn = container.querySelector('.cat-custom-btn');
        if (customBtn) {
            customBtn.addEventListener('click', function() { app.openCategoryManager(); });
        }
    }
...
// ========== 自定义分类管理 ==========
    openCategoryManager() {
        ...
        this._catManagerTab = 'expense';
        this.renderCatManagerList();
        this.hideCatForm();
        document.getElementById('cat-manager-close').onclick = function() { app.closeCategoryManager(); };
        overlay.onclick = function(e) { if (e.target === overlay) app.closeCategoryManager(); };
        document.querySelectorAll('.cat-manager-tab').forEach(function(tab) {
            tab.onclick = function() { app._catManagerTab = this.dataset.tab; app.renderCatManagerList(); };
        });
        document.getElementById('cat-manager-add-btn').onclick = function() { app.showCatForm(); };
        document.getElementById('cat-form-cancel').onclick = function() { app.hideCatForm(); };
        document.getElementById('cat-form-submit').onclick = function() { app.submitCatForm(); };
    }
    ...
    renderCatManagerList() {
        var tab = this._catManagerTab;
        document.querySelectorAll('.cat-manager-tab').forEach(function(t) {
            t.classList.toggle('active', t.dataset.tab === tab);
        });
        var cats = tab === 'income' ? this.getIncomeCategories() : this.getExpenseCategories();
        var list = document.getElementById('cat-manager-list');
        list.innerHTML = cats.map(function(c, idx) {
            var delBtn = c.isDefault ? '' : '<button class="cat-manager-item-delete" data-idx="' + idx + '">&times;</button>';
            var badge = c.isDefault ? '<span class="cat-manager-item-badge">默认</span>' : '';
            return '<div class="cat-manager-item">' +
                '<div class="cat-manager-item-icon" style="background:' + c.color + '20;color:' + c.color + '">' + c.icon + '</div>' +
                '<span class="cat-manager-item-name">' + c.name + '</span>' +
                badge +
                delBtn +
            '</div>';
        }).join('');
        list.querySelectorAll('.cat-manager-item-delete').forEach(function(btn) {
            btn.addEventListener('click', function(e) {
                e.stopPropagation();
                var idx = parseInt(btn.dataset.idx);
                app.deleteCustomCat(tab, idx);
            });
        });
    }

    showCatForm() {
        document.getElementById('cat-manager-form').classList.remove('hidden');
        document.getElementById('cat-manager-add-wrap').classList.add('hidden');
        document.getElementById('cat-form-name').value = '';
        document.getElementById('cat-form-type').value = this._catManagerTab;
        document.getElementById('cat-form-color').value = '#FF9500';
        this._selectedCatIcon = null;
        this.renderCatIconGrid();
    }

    hideCatForm() {
        document.getElementById('cat-manager-form').classList.add('hidden');
        document.getElementById('cat-manager-add-wrap').classList.remove('hidden');
    }

    renderCatIconGrid() {
        var icons = ['🍲','🚗','🛍','🎮','🏠','💊','📚','📦','💰','💵','💳','🐱','🐶','🌸','🎵','✈','🏃','🎁','📱','💻','📷','🎬','🍜','☕','🍺','🎂','🏥','📝','💡','🔧'];
        var grid = document.getElementById('cat-icon-grid');
        grid.innerHTML = icons.map(function(ico) {
            var cls = 'cat-icon-option' + (ico === app._selectedCatIcon ? ' selected' : '');
            return '<button class="' + cls + '" data-icon="' + ico + '">' + ico + '</button>';
        }).join('');
        grid.querySelectorAll('.cat-icon-option').forEach(function(btn) {
            btn.addEventListener('click', function() {
                app._selectedCatIcon = btn.dataset.icon;
                app.renderCatIconGrid();
            });
        });
    }
    ...
    submitCatForm() {
        ...
        var type = document.getElementById('cat-form-type').value;
        var color = document.getElementById('cat-form-color').value;
        var icon = this._selectedCatIcon || '📦';
        if (!name) return;
        // 检查重复
        var exists = this.customCategories.some(function(c) { return c.name === name && c.type === type; });
        if (exists) return;
        this.customCategories.push({ name: name, icon: icon, color: color, type: type, keywords: [] });
        this.saveCustomCategories();
        this.buildCategories();
        this.renderCatManagerList();
        this.hideCatForm();
    ...
    deleteCustomCat(tab, idx) {
        var cats = tab === 'income' ? this.getIncomeCategories() : this.getExpenseCategories();
        var target = cats[idx];
        if (!target || target.isDefault) return;
        this.customCategories = this.customCategories.filter(function(c) {
            return !(c.name === target.name && c.type === target.type);
        });
        this.saveCustomCategories();
        this.buildCategories();
        this.renderCatManagerList();
